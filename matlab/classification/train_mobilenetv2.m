function artifacts = train_mobilenetv2(varargin)
%TRAIN_MOBILENETV2 Train lightweight ICare DR classifier on APTOS.
%
% Examples:
%   train_mobilenetv2("quick", true)
%   train_mobilenetv2("maxEpochs", 20, "miniBatchSize", 16)
%   train_mobilenetv2("lossMode", "crossentropy")
...
fprintf("APTOS rows available for MobileNetV2 training: %d\n", height(tbl));
...
try
    if opts.pretrained
        ...
        baseNet = mobilenetv2(Weights="none");
        warning("ICare:MobileNetV2:Untrained", ...
            "Training MobileNetV2 from scratch because pretrained=false. This needs more data/epochs and is not transfer learning.");
    end
catch ME
    error("ICare:MobileNetV2:Unavailable", ...
        "mobilenetv2() pretrained weights are unavailable. Install 'Deep Learning Toolbox Model for MobileNet-v2 Network', or run train_mobilenetv2('pretrained', false). Original error: %s", ME.message);
end

lgraph = to_layer_graph(baseNet);
lgraph = replaceLayer(lgraph, "Logits", fullyConnectedLayer(numel(classNames), Name="icare_dr_logits", WeightLearnRateFactor=10, BiasLearnRateFactor=10));
...
lgraph = replaceLayer(lgraph, "ClassificationLayer_Logits", lossLayer);

modelRoot = string(cfg.modelRoot);
if ~isfolder(modelRoot)
    mkdir(modelRoot);
end
checkpointDir = fullfile(modelRoot, "checkpoints", "mobilenetv2");
...
artifactPath = fullfile(modelRoot, "mobilenetv2_dr_" + timestamp + ".mat");
if opts.quick
    latestPath = fullfile(modelRoot, "mobilenetv2_dr_quick.mat");
else
    latestPath = fullfile(modelRoot, "mobilenetv2_dr.mat");
end
metricsPath = fullfile(modelRoot, "mobilenetv2_dr_metrics_" + timestamp + ".mat");

modelVersion = "mobilenetv2-aptos-" + timestamp;
...
fprintf("Saved ICare MobileNetV2 model: %s\n", artifactPath);
